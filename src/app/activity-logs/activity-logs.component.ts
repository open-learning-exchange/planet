      ...
      .subscribe((logs) => {
        this.logs.data = this.logs.data.map((c: any) => {
          c['resource_views'] = logs[0].rows.find(l => {
            return (l => l.key[0] == c.code && l.key[1] == c.parentCode && l.key[2] === 'visit');
          });
          c['resource_downloads'] = logs[0].rows.find(l => {
            return (l => l.key[0] == c.code && l.key[1] == c.parentCode && l.key[2] === 'download');
          });
          c['user_logins'] = logs[1].rows.find(l => {
            return (l => l.key[0] == c.code && l.key[1] == c.parentCode && l.key[2] === 'login');
          });
          c['last_sync'] = logs[2].rows.find(l => {
            return (l => l.key[1] == c.code && l.key[2] == c.parentCode && l.key[0] === 'sync');
          });
          c['last_upgrade'] = logs[2].rows.find(l => {
            return (l => l.key[1] == c.code && l.key[2] == c.parentCode && l.key[0] === 'upgrade');
          });
          c['last_login'] = logs[2].rows.find(l => {
            return (l => l.key[1] == c.code && l.key[2] == c.parentCode && l.key[0] === 'login');
          });
          return c;
        })
