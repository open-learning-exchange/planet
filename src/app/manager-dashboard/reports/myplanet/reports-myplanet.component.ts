...
export class ReportsMyPlanetComponent extends MyPlanetFiltersBase implements OnInit {
  ...
  exportAll(): void {
    this.exportCsvHelper(this.planets, undefined, this.mapToCsvData, $localize`myPlanet Reports`);
  }

  exportSingle(planet: any): void {
    this.exportCsvHelper(planet.children, planet.name, this.mapToCsvData, $localize`myPlanet Reports for ${planet.name}`);
  }
