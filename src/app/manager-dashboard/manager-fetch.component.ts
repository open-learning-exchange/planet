...
export class ManagerFetchComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.isLoading = true;
    this.pushedItems.sortingDataAccessor = (data: any, sortHeaderId: string) => {
      if (sortHeaderId === 'item') {
        const title = data.db === 'courses' ? data.item.doc?.courseTitle : data.item.doc?.title;
