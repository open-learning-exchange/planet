...
describe('UsersService notifications', () => {
  ...
  it('cleans an associated account\'s tasks only under its own materialized ID', () => {
  ...
  it('deletes an associated account\'s rows without removing its home planet\'s native account from synchronized teams', () => {
    ...
    const canonicalOnLocalTeam = {
      _id: 'canonical-local-team', userId: 'org.couchdb.user:alex', userPlanetCode: 'planet-b', teamPlanetCode: 'planet-a'
    };
    const canonicalOnHomeTeam = {
      _id: 'canonical-home-team', userId: 'org.couchdb.user:alex', userPlanetCode: 'planet-b', teamPlanetCode: 'planet-b'
    };
