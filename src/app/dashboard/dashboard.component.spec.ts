...
describe('DashboardComponent', () => {
  ...
  const mockMembershipAndTeams = (memberships: any[], teams: any[]) => {
    couchServiceMock.findAll.mockImplementation((db: string, query: any) =>
      db === 'teams' ? of(query?.selector?.docType === 'membership' ? memberships : teams) : of([])
    );
  };
  ...
  it('keeps available teams when a membership points to a missing team document', () => {
    mockMembershipAndTeams([
      { teamId: 'existing-team', userId: mockUser._id, userPlanetCode: 'earth_code', docType: 'membership' },
      { teamId: 'missing-team', userId: mockUser._id, userPlanetCode: 'earth_code', docType: 'membership' }
    ], [ { _id: 'existing-team', name: 'Existing Team' } ]);
    createComponent();
    let teams: any[];
    ...
    expect(couchServiceMock.findAll).toHaveBeenCalledWith('teams', expect.objectContaining({
      selector: { _id: { $in: [ 'existing-team', 'missing-team' ] } }
    }));
    expect(teams.map(team => team._id)).toEqual([ 'existing-team' ]);
  });

  it('queries associated memberships under their canonical id and explicit origin', () => {
    const historical = {
      _id: 'membership-historical', teamId: 'team-1', userId: associatedUser._id, userPlanetCode: 'earth_code', docType: 'membership'
    };
    mockMembershipAndTeams([ historical ], [ { _id: 'team-1', name: 'Nation Team', teamPlanetCode: 'earth_code' } ]);
    createComponent(associatedUser);
    let teams: any[];
  ...
  it('accepts code-less memberships only when the team origin matches the account origin', () => {
    mockMembershipAndTeams([
      { _id: 'membership-community', teamId: 'community-team', teamPlanetCode: 'community', userId: 'org.couchdb.user:alex' },
      { _id: 'membership-nation', teamId: 'nation-team', teamPlanetCode: 'earth_code', userId: 'org.couchdb.user:alex' }
    ], [
    ...
    expect(couchServiceMock.findAll).toHaveBeenCalledWith('teams', expect.objectContaining({
      selector: { _id: { $in: [ 'community-team' ] } }
    }));
  ...
  it('preserves the explicit membership document when a code-less duplicate also survives', () => {
    ...
    mockMembershipAndTeams([
      { _id: 'membership-codeless', teamId: 'team-1', teamPlanetCode: 'community', userId: 'org.couchdb.user:alex', isLeader: true },
      explicit
    ], [
  ...
  it('prefers a code-less canonical membership over a historical materialized row', () => {
    const codeless = { _id: 'membership-codeless', teamId: 'team-1', teamPlanetCode: 'community', userId: 'org.couchdb.user:alex' };
    mockMembershipAndTeams([
      {
        _id: 'membership-historical', teamId: 'team-1', teamPlanetCode: 'community',
        userId: associatedUser._id, userPlanetCode: 'earth_code'
      },
      codeless
    ], [ { _id: 'team-1', name: 'Community Team', teamPlanetCode: 'community' } ]);
    createComponent(associatedUser);
    ...
    expect(teams[0].membershipDoc).toBe(codeless);
  });
