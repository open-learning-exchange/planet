...
import { identityPlanetCode, matchesUserIdentity } from '../shared/identity.utils';
...
export class TeamsMemberComponent implements OnInit, OnChanges {
  ...
  get isTeamLeader() {
    return !!this.teamLeader && memberCompare(this.member, this.teamLeader, this.planetCode);
  }

  get isSelf() {
    return matchesUserIdentity(this.member, this.user, this.planetCode);
  }

  get memberPlanetCode() {
    return identityPlanetCode(this.member);
  }
  ...
  openMemberDialog(member) {
    this.usersProfileDialogService.open({
      member: { ...member, userPlanetCode: identityPlanetCode(member) }
    });
  }
