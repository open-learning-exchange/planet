...
import { matchesUserIdentity, userIdentity } from '../shared/identity.utils';
...
export class TeamsViewComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  setTasks(tasks = []) {
    ...
    this.members = this.members.map(member => {
      const identities = this.matchesCurrentUser(member) ? currentUserTaskIdentities : [
  ...
  matchesCurrentUser(doc, user = this.user) {
    return matchesUserIdentity(doc, user, this.planetCode, this.team?.teamPlanetCode || this.planetCode);
  }
