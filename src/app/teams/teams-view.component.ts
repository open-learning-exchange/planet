...
export class TeamsViewComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  mode: "team" | "enterprise" | "services" =
    this.route.snapshot.data.mode || "team";
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe((params: ParamMap) => {
      this.teamId =
        params.get("teamId") ||
        `${this.stateService.configuration.code}@${this.stateService.configuration.parentCode}`;
      this.initTeam(this.teamId);
    });
    this.tasksService
      .tasksListener({ [this.dbName]: this.teamId })
      .subscribe(tasks => {
        this.tasks = tasks;
        this.setTasks(tasks);
      });
      ...
      .pipe(
        ...
        switchMap(() =>
          this.couchService.findAll(
            "team_activities",
            findDocuments({ teamId })
          )
        )
      )
      .subscribe(activities => {
        this.reportsService
          .groupBy(activities, ["user"], { maxField: "time" })
          .forEach(visit => {
            this.visits[visit.user] = {
              count: visit.count,
              recentTime: visit.max && visit.max.time
            };
          });
      ...
      switchMap((docs: any[]) => {
        const src = member => {
          ...
          if (member.attachmentDoc) {
            ...
            }/attachments/${userId}@${userPlanetCode}/${
              Object.keys(attachmentDoc._attachments)[0]
            }`;
          }
          if (member.userDoc && member.userDoc.doc._attachments) {
            return `${environment.couchAddress}/_users/${userId}/${
              Object.keys(userDoc.doc._attachments)[0]
            }`;
          }
        ...
        this.leader =
          (docsWithName.find(mem => mem.isLeader) || {}).userId ||
          this.team.createdBy;
        ...
        return this.teamsService.getTeamResources(
          docs.filter(doc => doc.docType === "resourceLink")
        );
      }),
  ...
  setTasks(tasks = []) {
    this.members = this.members.map(member => ({
      ...
      tasks: this.tasksService.sortedTasks(
        tasks.filter(
          ({ assignee }) => assignee && assignee.userId === member.userId
        ),
        member.tasks
      )
    }));
    if (this.userStatus === "member") {
      const tasksForCount =
        this.leader === this.user._id
          ? tasks
          : this.members.find(member => member.userId === this.user._id).tasks;
      this.taskCount = tasksForCount.filter(
  ...
  setStatus(team, user) {
    ...
    this.userStatus = this.isUserInMemberDocs(this.requests, user)
      ? "requesting"
      : this.userStatus;
    this.userStatus = this.isUserInMemberDocs(this.members, user)
      ? "member"
      : this.userStatus;
    if (
      this.initTab === undefined &&
      this.userStatus === "member" &&
      this.route.snapshot.params.activeTab
    ) {
  ...
  isUserInMemberDocs(memberDocs, user) {
    return memberDocs.some(
      (memberDoc: any) =>
        memberDoc.userId === user._id &&
        memberDoc.userPlanetCode === user.planetCode
    );
  }
    ...
    return () =>
      this.teamsService
        .toggleTeamMembership(
          team,
          leaveTeam,
          this.members.find(doc => doc.userId === this.user._id) || {
            userId: this.user._id,
            userPlanetCode: this.user.planetCode
          }
        )
        .pipe(
          switchMap(newTeam => {
  ...
  dialogPromptConfig(item, change) {
    return {
      leave: {
        request: this.toggleMembership(item, true),
        successMsg: "left",
        errorMsg: "leaving"
      },
      archive: {
        request: this.teamsService.archiveTeam(item),
        successMsg: "deleted",
        errorMsg: "deleting"
      },
      resource: {
        request: this.removeResource(item),
        name: item.resource && item.resource.title,
        successMsg: "removed",
        errorMsg: "removing"
      },
      course: {
        request: this.removeCourse(item),
        name: item.courseTitle,
        successMsg: "removed",
        errorMsg: "removing"
      },
      remove: {
        request: this.changeMembershipRequest("removed", item),
        name: (item.userDoc || {}).fullName || item.name,
        successMsg: "removed",
        errorMsg: "removing"
      },
      leader: {
        request: this.makeLeader(item),
        successMsg: "given leadership to",
        errorMsg: "giving leadership to"
      }
    }[change];
  }
  ...
  openDialogPrompt(
    ...
    const displayName =
      config.name || (item.userDoc ? item.userDoc.fullName : item.name);
        ...
        okClick: {
          ...
          onNext: res => {
            ...
            this.planetMessageService.showMessage(
              `You have ${config.successMsg} ${displayName}`
            );
          ...
          onError: () =>
            this.planetMessageService.showAlert(
              `There was a problem ${config.errorMsg} ${displayName}`
            )
        },
        displayName,
        ...dialogParams
  ...
  changeMembershipRequest(type, memberDoc?) {
    ...
    return () => {
      return changeObject.obs.pipe(
        switchMap(() =>
          type === "added"
            ? this.teamsService.removeFromRequests(this.team, memberDoc)
            : of({})
        ),
        switchMap(() =>
          type === "removed"
            ? this.tasksService.removeAssigneeFromTasks(memberDoc.userId, {
                teams: this.teamId
              })
            : of({})
        ),
        ...
        switchMap(() =>
          this.sendNotifications(type, {
            members: type === "request" ? this.members : [memberDoc]
          })
        ),
  ...
  private changeObject(type, memberDoc?) {
    const memberName =
      memberDoc && (memberDoc.userDoc.fullName || memberDoc.name);
    switch (type) {
      case "request":
        return {
      ...
      case "removed":
        return {
          obs: this.teamsService.toggleTeamMembership(
            this.team,
            true,
            memberDoc
          ),
  ...
  updateTeam() {
    this.teamsService
      .addTeamDialog(this.user._id, this.mode, this.team)
      .subscribe(updatedTeam => {
        this.team = updatedTeam;
        this.planetMessageService.showMessage(
          this.team.name ||
            `${this.configuration.name} Services Directory` +
  ...
  openInviteMemberDialog() {
    this.dialogsListService
      .getListAndColumns("_users")
      .pipe(takeUntil(this.onDestroy$))
      .subscribe(res => {
        res.tableData = res.tableData.filter(
          (user: any) =>
            this.members.findIndex(member => member.name === user.name) === -1
        );
        const data = {
  ...
  addMembers(selected: any[]) {
    ...
    const newMembershipDocs = selected.map(user =>
      this.teamsService.membershipProps(
        this.team,
        { userId: user._id, userPlanetCode: user.planetCode },
        "membership"
      )
    );
    const requestsToDelete = this.requests
      .filter(request =>
        newMembershipDocs.some(member => member.userId === request.userId)
      )
      .map(request => ({ ...request, _deleted: true }));
    this.couchService
      .bulkDocs(this.dbName, [...newMembershipDocs, ...requestsToDelete])
      .pipe(
        switchMap(() => {
          return forkJoin([
            ...
            this.sendNotifications("addMember", {
              newMembersLength: selected.length
            })
          ]);
        }),
        switchMap(() => this.getMembers()),
      ...
      .subscribe(() => {
        ...
        this.planetMessageService.showMessage(
          "Member" + (selected.length > 1 ? "s" : "") + " added successfully"
        );
      });
  }
  ...
  sendNotifications(
    type,
    { members, newMembersLength = 0 }: { members?; newMembersLength? } = {}
  ) {
      ...
      data: {
        okClick: (courses: any[]) => {
          ...
          this.teamsService
            .updateTeam({
              ...this.team,
              courses: [...(this.team.courses || []), ...newCourses].sort(
                (a, b) => a.courseTitle.localeCompare(b.courseTitle)
              )
            })
            .subscribe(updatedTeam => {
              this.team = updatedTeam;
    ...
    const dialogRef = this.dialog.open(DialogsAddResourcesComponent, {
      ...
      data: {
        okClick: (resources: any[]) => {
          this.teamsService
            .linkResourcesToTeam(resources, this.team)
        ...
        excludeIds: this.resources
          .filter(r => r.resource)
          .map(r => r.resource._id),
        canAdd: true,
        db: this.dbName,
        linkId: this.teamId,
        resource
  ...
  makeLeader(member) {
    const { tasks, ...currentLeader } = this.members.find(
      mem => mem.userId === this.leader
    );
    return () =>
      this.teamsService
        .changeTeamLeadership(currentLeader, member)
        .pipe(switchMap(() => this.getMembers()));
  }
  ...
  removeCourse(course) {
    ...
    return () =>
      this.teamsService.updateTeam({
        ...this.team,
        courses: this.team.courses.filter(c => c._id !== course._id)
      });
  }
