...
describe('TeamsViewComponent identity', () => {
  ...
  describe('membership and leadership', () => {
    ...
    it('resolves a code-less leader through the synchronized team origin', () => {
      ...
      component.setStatus(component.team, { userId: 'org.couchdb.user:ann', teamPlanetCode: 'community' }, component.user);
    ...
    it('cleans both current and pre-change task assignments for an unambiguous code-less member', () => {
      const component = build();
      const member = {
        _id: 'membership-1',
      ...
      expect(tasksService.removeAssigneeFromTasks).toHaveBeenCalledWith(
        member.userId, [ 'community', serverPlanet ], { teams: 'team-1' }
      );
    });

    it('keeps an old local-code task with the only matching code-less team member', () => {
      const component = build();
      const member = {
        userId: 'org.couchdb.user:bob', teamPlanetCode: 'community', docType: 'membership'
      };
      const oldTask = { _id: 'task-1', assignee: { userId: member.userId, userPlanetCode: serverPlanet } };
      component.members = [ member ];

      component.setTasks([ oldTask ]);

      expect(component.members[0].tasks).toEqual([ oldTask ]);
    });

    it('does not attribute an old local-code task to a same-id remote member when a local member exists', () => {
      const component = build();
      const remote = { userId: 'org.couchdb.user:bob', teamPlanetCode: 'community', docType: 'membership' };
      const local = { userId: remote.userId, userPlanetCode: serverPlanet, docType: 'membership' };
      const oldTask = { _id: 'task-1', assignee: { userId: remote.userId, userPlanetCode: serverPlanet } };
      component.members = [ remote, local ];

      component.setTasks([ oldTask ]);

      expect(component.members[0].tasks).toEqual([]);
      expect(component.members[1].tasks).toEqual([ oldTask ]);
    });

    it('cleans a member\'s historical materialized task identities when they are removed', () => {
      const component = build();
      const member = {
        _id: 'membership-1',
        userId: 'org.couchdb.user:ann',
        userPlanetCode: 'community',
        userDoc: { doc: associatedAnn }
      };
      component.team = { _id: 'team-1', teamPlanetCode: 'community' };
      component.teamId = 'team-1';
      component.getMembers = vi.fn().mockReturnValue(of([]));
      ...
      expect(tasksService.removeAssigneeFromTasks).toHaveBeenCalledWith(
        'org.couchdb.user:ann', expect.arrayContaining([ 'community' ]), { teams: 'team-1' }
      );
      expect(tasksService.removeAssigneeFromTasks).toHaveBeenCalledWith(
        'org.couchdb.user:ann@community', expect.arrayContaining([ 'community' ]), { teams: 'team-1' }
      );
    });
