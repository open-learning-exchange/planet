...
- `npm run lint:all` — ESLint + sass-lint (`lint:styles`) + htmlhint (`lint:html`). Not yet gating: the style/html baselines have pre-existing failures, so CI and the pre-push hook run only `npm run lint` until that debt is cleared.
