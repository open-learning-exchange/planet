...
- `gateway/` — standalone Express + WebSocket gateway with internal `chatapi`, `public`, and `sync` modules. It serves chat on the existing `/ml/` namespace and scoped public operations on `/api/`. Credentials/models come from the CouchDB `configurations` doc, not env vars. The `sync` module runs **timed syncs**: it reads `syncSchedule` off the local configuration doc and, when a sync is due, re-runs the replicator docs the last manual sync left in `_replicator` (see `gateway/README.md`). It needs `COUCHDB_USER`/`COUCHDB_PASS`, which the production entrypoint already writes into the gateway container.
