...
`TRUST_PROXY_CLIENT_IP=true` makes pre-auth limits use nginx's `X-Real-IP` header. Enable it only when the gateway is not directly reachable by untrusted clients and the proxy overwrites that header. The production Compose topology meets those conditions; the development override publishes the gateway port and disables header trust. When Planet itself is behind another reverse proxy, set `PLANET_TRUSTED_PROXY_CIDR` on the Planet container to that proxy's address or network and configure the outer proxy to overwrite `X-Forwarded-For`; otherwise all proxied clients correctly remain untrusted but share the outer proxy's address.
