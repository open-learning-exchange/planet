...
# CHAT_MAX_CONVERSATION_TURNS=40
# CHAT_HISTORY_REPLAY_TURNS=20
# RESOURCE_INDEX_MAX_FILES=50
# RESOURCE_INDEX_MAX_FILE_BYTES=26214400
# RESOURCE_INDEX_MAX_TOTAL_BYTES=52428800
# RESOURCE_INDEX_RECONCILIATION_START_DELAY_MS=300000
```
...
`RESOURCE_INDEX_TIMEOUT_MS` bounds resource-lock waits, CouchDB reads/writes, uploads, and file-batch polling during index construction; it defaults to ten minutes and is also used by the dedicated OpenAI indexing client. Failed-build cleanup has its own short maintenance deadline. `COUCHDB_SESSION_TIMEOUT_MS` bounds session validation and defaults to ten seconds. `GATEWAY_JSON_LIMIT` bounds parsed JSON request bodies and defaults to 1 MiB. AI resource indexing accepts at most 50 supported attachments and defaults to 25 MiB per attachment and 50 MiB total per resource; `RESOURCE_INDEX_MAX_FILES`, `RESOURCE_INDEX_MAX_FILE_BYTES`, and `RESOURCE_INDEX_MAX_TOTAL_BYTES` override those limits. `RATE_LIMIT_PER_MINUTE=0` is a ChatAPI kill switch. Positive values set the per-user request ceiling; individual routes may use a lower ceiling. `PRE_AUTH_RATE_LIMIT_PER_MINUTE` defaults to 120 requests per IP and route, limiting unauthenticated requests before session validation. Resource cleanup is limited to five batches per minute.

`TRUST_PROXY_CLIENT_IP=true` makes pre-auth limits use nginx's `X-Real-IP` header. Enable it only when the gateway is not directly reachable by untrusted clients and the proxy overwrites that header. The production Compose topology meets those conditions; the development override publishes the gateway port and disables header trust. When Planet itself is behind another reverse proxy, set the single-value `PLANET_TRUSTED_PROXY_CIDR` on the Planet container to that proxy's address or network and configure the outer proxy to overwrite `X-Forwarded-For` and `X-Forwarded-Proto`; otherwise all proxied clients correctly remain untrusted but share the outer proxy's address. Never configure an unrestricted trusted CIDR.
...
`providers/registry.ts` is the single source of provider identity: each entry holds the display label, API transport, OpenAI-compatible base URL, and explicitly supported capabilities. Adding a chat provider there is enough for it to appear in `/checkproviders`, get a client built from the `configurations` keys and models, and render its own API key and model fields in Manager AI Services — the Angular app no longer keeps its own provider list. The `fileSearch` capability additionally requires compatibility with OpenAI's vector-store, file, and file-search APIs; ordinary Chat Completions compatibility is not sufficient.

Empty or whitespace-only prompt-profile fields use the gateway's built-in instructions; a configured value is trimmed and replaces the built-in instructions for that mode. Clearing the fields in Manager AI Services and saving restores the built-in profiles. The Angular client sends its locale, which the gateway maps to a known response language without accepting arbitrary client-supplied instructions. `/checkproviders` returns provider availability and these built-in defaults so the manager UI can show exactly what an override replaces.

Saved conversations retain at most 40 successful turns by default. Provider requests replay only the latest 20 turns, while the complete persisted history remains available to Planet. `CHAT_MAX_CONVERSATION_TURNS` and `CHAT_HISTORY_REPLAY_TURNS` override those limits; replay is always capped by the persisted-turn limit. Older turns are not AI-summarized.

Resource-index cleanup starts on a best-effort basis while the resource still exists for ownership checks, but the Planet deletion never waits or warns when cleanup is unavailable. The gateway retains failed cleanup state for reconciliation. Reconciliation starts five minutes after boot, persists its last successful scan in a deployment-local CouchDB document, and runs no more than once every 24 hours; `RESOURCE_INDEX_RECONCILIATION_START_DELAY_MS` changes only the initial delay.
